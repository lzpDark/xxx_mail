...
public class BuilderProcessor extends AbstractProcessor {
    ...
    public boolean process(Set<? extends TypeElement> annotations, RoundEnvironment roundEnv) {
        ...
        for (TypeElement annotation : annotations) {
            ...
            Map<Boolean, ? extends List<? extends Element>> annotatedMethods = annotatedElements.stream().collect(Collectors.partitioningBy(element -> (((ExecutableType) element.asType()).getParameterTypes().size() == 1 && element.getSimpleName().toString().startsWith("set"))));
            ...
            String className = ((TypeElement) setters.get(0).getEnclosedElements()).getQualifiedName().toString();
